...
void IGPJpeg::njRowIDCT(int* blk) {
...
void IGPJpeg::njColIDCT(const int* blk, unsigned char *out, int stride) {
	int x0, x1, x2, x3, x4, x5, x6, x7, x8;
	if (!((x1 = blk[8 * 4] << 8)
		| (x2 = blk[8 * 6])
		| (x3 = blk[8 * 2])
...
int IGPJpeg::njShowBits(int bits) {
...
void IGPJpeg::njSkipBits(int bits) {
	if (nj.bufbits < bits)
		(void) njShowBits(bits);
	nj.bufbits -= bits;
}

int IGPJpeg::njGetBits(int bits) {
	int res = njShowBits(bits);
	njSkipBits(bits);
	return res;
}

void IGPJpeg::njByteAlign(void) {
	nj.bufbits &= 0xF8;
}

void IGPJpeg::njSkip(int count) {
	nj.pos += count;
	nj.size -= count;
	nj.length -= count;
...
unsigned short IGPJpeg::njDecode16(const unsigned char *pos) {
	return (pos[0] << 8) | pos[1];
}

void IGPJpeg::njDecodeLength(void) {
...
void IGPJpeg::njSkipMarker(void) {
	njDecodeLength();
	njSkip(nj.length);
}

void IGPJpeg::njDecodeSOF(int decodeRed, int decodeGreen, int decodeBlue) {
...
int IGPJpeg::njGetVLC(nj_vlc_code_t* vlc, unsigned char* code) {
...
void IGPJpeg::njDecodeBlock(nj_component_t* c, unsigned char* out) {
...
void IGPJpeg::njDecodeScan(void) {
...
void IGPJpeg::njUpsampleH(nj_component_t* c) {
...
void IGPJpeg::njUpsampleV(nj_component_t* c) {
...
void IGPJpeg::njUpsample(nj_component_t* c) {
...
void IGPJpeg::njConvert(int decodeRed, int decodeGreen, int decodeBlue) {
	int i;
	nj_component_t* c;
	for (i = 0, c = nj.comp; i < nj.ncomp; ++i, ++c) {
#if NJ_CHROMA_FILTER
...
void IGPJpeg::njInit(void) {
	njFillMem(&nj, 0, sizeof(nj_context_t));
}

void IGPJpeg::njDone(void) {
	int i;
	for (i = 0; i < 3; ++i)
...
void IGPJpeg::njDoneLeaveRGBData(void) {
	int i;
	for (i = 0; i < 3; ++i) {
		if (nj.comp[i].pixels) {
...
nj_result_t IGPJpeg::njDecode(const void* jpeg, const int size, int decodeRed, int decodeGreen, int decodeBlue) {
	njDone();
